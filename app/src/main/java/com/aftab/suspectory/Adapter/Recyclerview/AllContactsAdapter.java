...
import com.aftab.suspectory.Interface.AddToSuspected;
...
public class AllContactsAdapter extends RecyclerView.Adapter<AllContactsAdapter.VH> implements FastScroller.SectionIndexer {

    Context context;
    ArrayList<Contacts> contactsList;
    AddToSuspected addToSuspected;

    public AllContactsAdapter(Context context, ArrayList<Contacts> contactsList, AddToSuspected addToSuspected) {
        this.context = context;
        this.contactsList = contactsList;
        this.addToSuspected = addToSuspected;
    }

    @NonNull
    @Override
    public VH onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.all_contacts_item, parent, false);
    ...
    public void onBindViewHolder(@NonNull VH holder, int position) {
        ...
        holder.ivAdd.setOnClickListener(v -> addToSuspected.addToSuspected(contactsList.get(position)));

        holder.itemView.setOnClickListener(v -> {

            Intent intent = new Intent(context, ChatActivity.class);
            intent.putExtra(Constants.FROM, Constants.CONTACTS);
    ...
    private void setData(VH holder, Contacts contacts) {
    ...
    public CharSequence getSectionText(int i) {
        String fChar = contactsList.get(i).getName().substring(0, 1);
        return fChar;
    }

    public static class VH extends RecyclerView.ViewHolder {

        TextView tvFirstChar, tvName, tvPhone, tvUnReadCount;
        ImageView ivAdd;
